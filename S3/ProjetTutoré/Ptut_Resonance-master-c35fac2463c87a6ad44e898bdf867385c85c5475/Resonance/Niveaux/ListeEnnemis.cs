...
{
    class ListeEnnemis : Liste
    {
        public ListeEnnemis(Color colorText, Vector2 Position, ArrayList DataList) : base(Position, DataList)
        {
            foreach (Ennemis Data in DataListe)
            {
        ...
        {
            bouton.Texture = content.Load<Texture2D>("icone_ennemi");
            bouton.longueur = bouton.Texture.Width;
            bouton.largeur = bouton.Texture.Height;
            texturetype = content.Load<Texture2D>("texturetype_ennemis");
