...
{
    public partial class Ajouter_Ennemi : Form
    {
        OpenFileDialog OFD;
        String[] NomFichier;
        ...
        private void Valider_Click(object sender, EventArgs e)
        {
            ...
            if (ext_valide && Nom.Text.Length > 0)
            {
                ...
                {
                    System.IO.File.Copy(NomFichier[i], String.Concat("Content\\Ennemis\\", Nom.Text, Img_Liste[i].Name.Substring(Img_Liste[i].Name.IndexOf('_')) , NomFichier[i].Substring(NomFichier[i].LastIndexOf('.'))), true);
                }
                int nblignes = Editeur.Nombre_lignes_fichier("BD_Ennemis.txt") + 1;
                String identifiant;
                if (nblignes < 100)
                    if (nblignes < 10)
                ...
                StreamWriter SR = new StreamWriter(String.Concat(Niveau.Root, "BD_Ennemis.txt"), true);
