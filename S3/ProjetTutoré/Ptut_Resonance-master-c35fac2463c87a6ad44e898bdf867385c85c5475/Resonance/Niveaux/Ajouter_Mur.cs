...
{
    public partial class Ajouter_Mur : Form
    {
        OpenFileDialog OFD;
        String NomFichier;
        public Ajouter_Mur()
        ...
        private void Valider_Click(object sender, EventArgs e)
        {
            ...
            if (File.Exists(Emplacement.Text) && ext_valide && Nom.Text.Length > 0)
            {
                ...
                {
                    System.IO.File.Copy(NomFichier, String.Concat("Content\\Murs\\", Nom.Text, NomFichier.Substring(NomFichier.LastIndexOf('.'))), true);
                    int nblignes = Editeur.Nombre_lignes_fichier("BD_Murs.txt") + 1;
                    String identifiant;
                    if (nblignes < 100)
                        if (nblignes < 10)
                    ...
                    StreamWriter SR = new StreamWriter(String.Concat(Niveau.Root, "BD_Murs.txt"), true);
